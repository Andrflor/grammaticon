...
class TheatrumGame extends EncounterScene {
  TheatrumGame({required this.opponentId, super.reducedMotion = false, this.background = 'theatrum_bg', this.heroPrefix = 'histrio', this.opponentPrefix = 'actor_'});

  final String opponentId;

  /// Asset names: background, hero pose prefix (`<prefix>_idle`…), opponent prefix.
  final String background;
  final String heroPrefix;
  final String opponentPrefix;
  ...
  Future<void> onLoad() async {
    final names = [background, '${heroPrefix}_idle', '${heroPrefix}_gesture', '${heroPrefix}_hurt', '${heroPrefix}_victory', '${heroPrefix}_defeat', '$opponentPrefix$opponentId', 'persona', 'laurel'];
    ...
    histrio = Fighter(idle: _sprites['${heroPrefix}_idle']!, facingRight: true, priority: 2);
    opponent = Fighter(idle: _sprites['$opponentPrefix$opponentId']!, facingRight: false, priority: 1);
  ...
  void playerStrikes() {
    if (!_ready) return;
    histrio.swapSprite(_sprites['${heroPrefix}_gesture']!, const Duration(milliseconds: 640));
  ...
  void opponentStrikes() {
    ...
    Future<void>.delayed(const Duration(milliseconds: 240), () {
      if (!isMounted) return;
      histrio.swapSprite(_sprites['${heroPrefix}_hurt']!, const Duration(milliseconds: 700));
    });
    if (reducedMotion) {
  ...
  void victory() {
    if (!_ready) return;
    histrio.swapSprite(_sprites['${heroPrefix}_victory']!, const Duration(days: 1));
    opponent.defeated();
    if (!reducedMotion) {
  ...
  void defeat() {
    if (!_ready) return;
    histrio.swapSprite(_sprites['${heroPrefix}_defeat']!, const Duration(days: 1));
    if (!reducedMotion) {
