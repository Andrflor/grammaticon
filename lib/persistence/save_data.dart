...
class SaveData {
  const SaveData({
    ...
    this.arbor = const ArborEvidence(),
    this.createdAt,
    this.updatedAt,
  });
  ...
  /// Évidence par maillon de l'arbre des compétences.
  final ArborEvidence arbor;
  ...
  factory SaveData.fromJson(Map<String, Object?> j) => SaveData(
    ...
    arbor: j['arbor'] == null ? const ArborEvidence() : ArborEvidence.fromJson((j['arbor'] as Map).cast<String, Object?>()),
...
class SaveCodec {
  ...
  /// Sauvegarde du moteur « design JSON » (10–14 septembre 2026). Elle porte
  /// sous `legacy` la sauvegarde typée antérieure complète : on repart de
  /// celle-ci, puis on y reporte ce qui a été gagné depuis — solde, achats,
  /// leçons vues, statistiques, réglages. Les compétences du moteur JSON
  /// (67 021 nœuds par forme) ne se projettent pas et ne sont pas reprises.
  SaveData fromPivot(Map<String, Object?> j) {
    var base = const SaveData();
    final legacy = j['legacy'];
    if (legacy is Map && legacy['schema'] != null) {
      try {
        base = decode(jsonEncode(legacy));
      } catch (_) {
        base = const SaveData();
      }
    }
    String? trialOf(Object? address) {
      if (address is! String) return null;
      final p = address.split('/');
      if (p.length < 3) return null;
      return switch (p[0]) {
        'amphitheatrum' || 'forum' => p.last,
        'theatrum' => 'th-${p[1]}-${p[2]}',
        'templum' => 'tp-${p[1]}-${p[2]}',
        _ => null,
      };
    }
    final purchased = {...base.purchased, for (final a in (j['purchased'] as List? ?? const [])) ?trialOf(a)};
    final introSeen = {...base.introSeen, for (final a in (j['seenLessons'] as List? ?? const [])) ?trialOf(a)};
    final stats = Map<String, ActivityStats>.from(base.activityStats);
    var won = base.battlesWon, lost = base.battlesLost;
    for (final e in ((j['statistics'] as Map?) ?? const {}).entries) {
      final v = (e.value as Map?) ?? const {};
      final w = (v['won'] as num?)?.toInt() ?? 0, l = (v['lost'] as num?)?.toInt() ?? 0;
      final cur = stats[e.key as String] ?? const ActivityStats();
      stats[e.key as String] = ActivityStats(won: cur.won + w, lost: cur.lost + l);
      won += w;
      lost += l;
    }
    final s = (j['settings'] as Map?) ?? const {};
    final settings = base.settings.copyWith(
      soundOn: s['sound'] as bool?,
      musicOn: s['music'] as bool?,
      volume: (s['volume'] as num?)?.toDouble(),
      musicVolume: (s['musicVolume'] as num?)?.toDouble(),
      reducedMotion: s['reducedMotion'] as bool?,
      correctDelayMs: (s['correctDelayMs'] as num?)?.toInt(),
    );
    return base.copyWith(
      gems: (j['balance'] as num?)?.toInt() ?? base.gems,
      purchased: purchased,
      introSeen: introSeen,
      activityStats: stats,
      battlesWon: won,
      battlesLost: lost,
      settings: settings,
      lastTransactionId: [base.lastTransactionId, (j['transaction'] as num?)?.toInt() ?? 0].reduce((a, b) => a > b ? a : b),
  ...
  SaveData decode(String raw) {
    ...
    if (j['designId'] != null && j['schema'] == null) return fromPivot(j);
