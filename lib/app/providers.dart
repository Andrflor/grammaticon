...
final frameQuestionSourceProvider = Provider<FrameQuestionSource>((ref) => FrameQuestionSource(ref.watch(frameLibraryProvider)));
...
final questionSourcesProvider = Provider<QuestionSources>(
  (ref) => QuestionSources((a) => switch (a) {
        ...
        Activity.theatrum || Activity.templum => ref.read(frameQuestionSourceProvider),
      }),
);
...
class ProfileController extends Notifier<SaveData> {
  ...
  Future<void> applyResolution(Resolution r, ActiveBattle? snapshot) {
    ...
    return _commit(
      state.copyWith(gems: r.gemsAfter, skills: r.skillsAfter, lemmaDaily: r.lemmaDailyAfter, lastTransactionId: r.transaction.id, activeBattle: snapshot, clearActiveBattle: snapshot == null, exposure: r.exposureAfter, errata: r.errataAfter, arbor: r.arborAfter),
    );
  }
