...
class G {
 ...
 Color paint(String token) => Color(int.parse(data['paintedPalette'][token] as String, radix:16));
 ...
 List<Color> get inscriptionGold => [paint('FFFFF3B0'),paint('FFF8DE84'),paint('FFE8BF58')];
 List<Shadow> get headingShadow => [Shadow(color:paint('FF24103F'),offset:const Offset(0,2),blurRadius:2),Shadow(color:paint('FF200A40'),blurRadius:5),Shadow(color:paint('CC200A40'),blurRadius:14)];
  /// Small labels were hard to read: every size below 16 is raised by 3 points.
  double _readable(double size) => size < 16 ? size + 3 : size;

  TextStyle display(double size, {Color? color, double weight = 700, double letterSpacing = 1.2}) =>
      TextStyle(fontFamily: headingFont, fontSize: _readable(size), color: color ?? gold, fontVariations: [FontVariation('wght', weight)], letterSpacing: letterSpacing, height: 1.15);

  TextStyle body(double size, {Color? color, double weight = 500, FontStyle style = FontStyle.normal, double height = 1.3}) =>
      TextStyle(fontFamily: bodyFont, fontSize: _readable(size), color: color ?? ink, fontVariations: [FontVariation('wght', weight)], fontStyle: style, height: height);
  ...
  ThemeData theme() {
    ...
    return base.copyWith(
      ...
      textTheme: base.textTheme.apply(fontFamily: bodyFont, bodyColor: ink, displayColor: ink),
      ...
      switchTheme: SwitchThemeData(
        thumbColor: WidgetStateProperty.resolveWith((s) => s.contains(WidgetState.selected) ? gold : paint('FFE8DCBF')),
        trackColor: WidgetStateProperty.resolveWith((s) => s.contains(WidgetState.selected) ? paint('FF6834B2') : paint('FF7E7A82')),
