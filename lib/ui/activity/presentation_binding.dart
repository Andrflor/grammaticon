...
class BattleView {
  ...
  BattlePhase get phase => switch (data['phase']) {
    ...
    'feedback' =>
      data['lastCorrect'] == true ? BattlePhase.correct : BattlePhase.wrong,
  ...
  QuestionView? get question => session.question == null
      ? null
      : QuestionView(session, session.question!);
  OutcomeView? get last => data['chosen'] == null || question == null
      ? null
      : OutcomeView(this, question!);
}
...
class QuestionView {
  ...
  List<ChoiceView> get choices => entry.choices
      .map((c) => ChoiceView(c['id'] as String, session.text(c['text'])))
      .toList();
...
class OutcomeView {
  ...
  int get gemsDelta {
    ...
    return observations.isEmpty
        ? 0
        : observations.last['balanceDelta'] as int? ?? 0;
  }

  String get explanation =>
      state.session.text(question.entry.outcome(chosenValue)['feedback']);
}
