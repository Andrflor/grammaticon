...
class BattleScreen extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final game = useMemoized(
      () => trial.presentation['motion'] == 'projectile'
          ? ProjectileScene(
              design: session.design,
              view: trial.presentation,
              skin: skin,
    ...
    final startTiers = useMemoized(
      () => {for (final id in trial.skillIds) id: session.level(id)},
      [session.encounter?['id']],
    );
    final flights = useState<List<_GemFlightSpec>>([]);
    final timers = useRef(<Timer>[]);
    final lesson = useFuture(
      useMemoized(() => session.design.lesson(node), [trial.id]),
    );
    final currencyAsset = session.design.asset(
      session.design.root['presentation']['currency'],
    );
    final lastAnswer = useRef(state.answered);
    final lastPhase = useRef(state.phase);
    void later(int ms, VoidCallback action) {
      timers.value.add(Timer(Duration(milliseconds: ms), action));
    }
    ...
    useEffect(() {
          ...
          if (state.last!.gemsDelta > 0 && !reduced) {
            ...
            if (from != null && to != null) {
              ...
              later(
                ...
                () => flights.value = [
                  ...flights.value,
                  _GemFlightSpec(id, from, to, min(state.last!.gemsDelta, 8)),
                ],
              );
              later(
                1500,
      ...
      if (lastPhase.value != state.phase) {
        ...
        if (state.phase == BattlePhase.intro ||
            state.phase == BattlePhase.question) {
          game.resetPoses();
        }
      }
      return null;
    ...
    useEffect(() {
      bool handler(KeyEvent e) {
        ...
        if (digit != null &&
            state.acceptsInput &&
            digit <= state.question!.choices.length) {
          session.answer(state.question!.choices[digit - 1].value);
          return true;
        }
        return false;
      }
    ...
    return PopScope(
      ...
      child: Focus(
        autofocus: true,
        child: Scaffold(
          backgroundColor: skin.purpleDark,
          body: Stack(
            fit: StackFit.expand,
            children: [
              ...
              if (state.phase == BattlePhase.intro && lesson.hasData)
                _IntroOverlay(
                  skin: skin,
                  trial: trial,
                  lesson: trial.presentation['introductionBlocks'] == null
                      ? objects(lesson.data)
                      : [
                          for (final i
                              in trial.presentation['introductionBlocks']
                                  as List)
                            objects(lesson.data)[i as int],
                        ],
  ...
  static int? _digitOf(KeyEvent e) {
    ...
    if (k.keyId >= LogicalKeyboardKey.digit1.keyId &&
        k.keyId <= LogicalKeyboardKey.digit9.keyId) {
      return k.keyId - LogicalKeyboardKey.digit0.keyId;
    }
    if (k.keyId >= LogicalKeyboardKey.numpad1.keyId &&
        k.keyId <= LogicalKeyboardKey.numpad9.keyId) {
      return k.keyId - LogicalKeyboardKey.numpad0.keyId;
    }
...
class _Hud extends StatelessWidget {
  String get currencyAsset => state.session.design.asset(
    state.session.design.root['presentation']['currency'],
  );
  final G skin;
  const _Hud({
    required this.skin,
  ...
  Widget build(BuildContext context) {
    ...
    final bar = Column(
      children: [
        Text(
          enemyName,
          style: skin.display(14, color: skin.goldLight),
          maxLines: 1,
        ...
        Text(
          '${resource.isEmpty ? '' : '$resource '}${state.enemyHp} / ${state.enemyMaxHp}',
          style: skin.body(11, color: Colors.white, weight: 700),
        ),
      ],
    );
    final controls = Row(
      ...
      children: [
        ...
        RomanButton(
          ...
          style: RomanButtonStyle.ghost,
          dense: true,
          onPressed: state.isOver ? null : onPause,
        ),
        SizedBox(width: 10),
        for (var i = 0; i < state.maxHearts; i++)
          Padding(
            ...
            child: Image.asset(
              i < state.hearts
                  ? state.session.design.asset(
                      state.session.design.root['presentation']['life'],
                    )
                  : state.session.design.asset(
                      state.session.design.root['presentation']['emptyLife'],
                    ),
              width: 30,
              height: 30,
            ),
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            if (compact)
              Padding(padding: EdgeInsets.fromLTRB(40, 4, 40, 0), child: bar),
          ],
        ),
      ),
    );
...
class _Center extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (q == null || state.phase == BattlePhase.intro || state.isOver) {
      return SizedBox.shrink();
    }
    final outcome =
        state.phase == BattlePhase.correct || state.phase == BattlePhase.wrong
        ? state.last
        : null;
    ...
    final surfaceSize = long
        ? (compact
              ? (q.surface.length > 140 ? 15.0 : 17.0)
              : (q.surface.length > 160
                    ? 19.0
                  ...
                  ? Row(
                      ...
                      children: [
                        Text(
                          ...
                          style: skin.display(
                            compact ? 26 : 34,
                            color: outcome.correct ? skin.green : skin.red,
                          ),
                        ),
                        if (outcome.gemsDelta != 0) ...[
                          SizedBox(width: 12),
                          Text(
                            '${outcome.gemsDelta > 0 ? '+' : ''}${outcome.gemsDelta}',
                            style: skin.display(
                              compact ? 22 : 28,
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: _Choices(
                        ...
                        q: q,
                        state: state,
                        onPick: (i) => ctrl.answer(q.choices[i].value),
                      ),
                    ),
                  ],
                ),
...
class _Choices extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final perRow = sentences
        ? (w < 900 ? 1 : 2)
        : (w < 600 ? (n <= 2 ? n : (longest > 9 ? 1 : 2)) : (n <= 4 ? n : 3));
    final bw =
        ((min(w, sentences ? 1200 : 1000) - 24) - (perRow - 1) * 10) / perRow;
    return Wrap(
        ...
        for (var i = 0; i < n; i++)
          SizedBox(
            ...
            child: _ChoiceButton(
              ...
              isCorrect:
                  answered && q.correctValues.contains(q.choices[i].value),
              isChosen: answered && outcome.chosenValue == q.choices[i].value,
              wrong:
                  answered &&
                  outcome.chosenValue == q.choices[i].value &&
                  !outcome.correct,
              dense: sentences,
              onTap: () => onPick(i),
            ),
...
class _ChoiceButton extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final style = isCorrect
        ? RomanButtonStyle.success
        : (wrong ? RomanButtonStyle.danger : RomanButtonStyle.primary);
      ...
      child: AnimatedScale(
        ...
        child: RomanButton(
          ...
          trailing: isCorrect
              ...
              : (wrong ? Icon(Icons.cancel, color: Colors.white) : null),
        ),
      ),
    );
  }
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Text(
              trial.subtitle,
              style: skin.body(14, color: skin.inkSoft, weight: 700),
            ),
            SizedBox(height: 10),
            Text(
              lesson
                  .where((b) => b['type'] == 'text')
                  .map((b) => trial.session.text(b['text']))
                  .join('\n\n'),
            ...
            for (final e
              ...
              Padding(
                ...
                child: RomanPanel(
                  ...
                  child: Text(
                    e,
                    style: skin.body(17, weight: 700, color: skin.purpleDark),
                  ),
                ),
              ),
            SizedBox(height: 14),
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    '${trial.hearts} ${trial.session.label('labels.introductionLives')}. ${trial.session.text(trial.session.design.rules['economy']['introductionDescription'])}',
                    style: skin.body(13, color: skin.redDark, weight: 700),
                  ),
                ),
              ],
            ),
            ...
            Row(
              ...
              children: [
                RomanButton(
                  skin: skin,
                  label: trial.session.label('actions.start'),
  ...
  Widget build(BuildContext context) => _Dim(
    ...
    child: RomanPanel(
      ...
      child: Column(
        ...
        children: [
          Text(
            session.label('actions.pause'),
            style: skin.display(28, color: skin.purple),
          ),
          SizedBox(height: 8),
          ...
          Wrap(
            ...
            children: [
              RomanButton(
                skin: skin,
                label: session.label('actions.continue'),
                icon: Icons.play_arrow,
                style: RomanButtonStyle.gold,
                onPressed: onResume,
              ),
              RomanButton(
                skin: skin,
                label: session.label('actions.leave'),
      ...
      child: RomanPanel(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 14),
            _line(
              session.label('labels.correctAnswers'),
              '${state.correctCount} / ${state.answered}',
            ),
            _line(
              config.label('gemsLine'),
              '${state.gemsDelta >= 0 ? '+' : ''}${state.gemsDelta}',
            ),
            if (won)
              _line(
                session.label('labels.victoryBonus'),
                '+${state.victoryBonus}',
              ),

            if (!won)
              _line(config.label('penaltyLine'), '−${state.defeatPenalty}'),
            if (!won)
              Padding(
            ...
            SizedBox(height: 10),
            Text(
              session.label('labels.skills'),
              style: skin.display(15, color: skin.goldDark),
            ),
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    Expanded(
                      child: Text(
                        session.text(session.design.knowledge[s]?['name']),
                        style: skin.body(14, weight: 700),
                      ),
                    ),
                    LevelBadge(
                      session: session,
                      skin: skin,
                      level: startTiers[s] ?? 0,
                      dense: true,
                    ),
                    Padding(
            ...
            Wrap(
              ...
              children: [
                ...
                RomanButton(
                  skin: skin,
                  label: session.label('actions.retry'),
                  icon: Icons.replay,
...
class _GemFlight extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final offsets = useMemoized(
      () => [
        for (var i = 0; i < spec.count; i++)
          Offset((rnd.nextDouble() - 0.5) * 120, (rnd.nextDouble() - 0.5) * 60),
      ],
    );
              ...
              for (var i = 0; i < spec.count; i++)
                Builder(
                  builder: (context) {
                    ...
                    final ctrl = Offset(
                      (start.dx + spec.to.dx) / 2,
                      min(start.dy, spec.to.dy) - 140,
                    );
                    ...
                    return Positioned(
                      ...
                      top: p.dy - 14,
                      child: Opacity(
                        opacity: (t < 0.95 ? 1.0 : 1 - (t - 0.95) / 0.05).clamp(
                          0.0,
                          1.0,
                        ),
