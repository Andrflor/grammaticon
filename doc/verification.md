...
### Opening regrouping, adaptive selection and combat termination

Applied the user's revised direction: the opening is four broader cards instead of twelve. In Italiā, Ūnus et plūrēs, Magnus, magna, magnum and Affirmātiō et negātiō each have 32 explicit sentence questions per direction. The terminal card covers the 28 lemmas actually encountered. No duplicate prompt or duplicate choice text remains within these opening banks. Removed the old tiny-card directories and relocated existing source evidence only for retained questions. Existing actors and temple opponents are reused across the four cards. The following group depends on the actual final opening card. Only the first card is free; subsequent opening cards cost 20 gems.

The number card keeps singular choices first and plural choices second. Replaced absurd spatial-inversion distractors with meaningful number, agreement or negation contrasts. Choice ordering is now configurable: a question's shuffleChoices overrides its card's value, with false as the default. Shuffling changes only the stored presentation order; accepted IDs and diagnostics are preserved, and saved encounters resume with the same ordering. The catalog checker no longer imposes balanced answer positions, because that would contradict explicitly authored stable grammatical keys.

All Theatrum/Templum banks now allow adaptive engine selection instead of fixed next chains. Individual question results influence weights: unanswered questions receive a configurable boost, successes reduce repetition pressure, and active errors retain their configured priority. Recent questions persist across encounters. This is weighted selection based on the learner's record, not a mandatory pass through the whole bank. Explicit pedagogical links remain entirely authored; the engine does not infer subject relationships. The remaining small banks outside the opening still need expansion and their grouping needs continued review.

Fixed combat termination so that reaching the displayed target ends even a legacy sequence before another question. Zero-life and zero-target encounters cannot accept answers; restoring or resuming an exhausted encounter resolves its outcome, and repeated terminal actions do not award rewards again. Feedback for the final answer can still be read before continuing to the result screen. No scene, layout or drawing code changed; the presentation adapter additionally rejects input in exhausted state.

Also replaced the old mixed future-report card with six original questions contrasting future-in-the-past, simultaneous action and prior action. It now belongs in Verba relāta, after the present-report future card; both affected vocabulary inventories and prerequisite chains were rebuilt. That card and the other advanced banks still require additional variety.

All 38 Flutter tests pass, including zero-resource restores, three-error defeat, sequence target termination, adaptive relevance, recent-question persistence, optional choice shuffling, saved choice ordering, economy, mastery and graphics. Complete design validation passes with 311 cards and 892,362 authored questions across Grammaticon. Theatrum/Templum contain 140 cards and 1,808 questions. Catalog checks, lexical traceability freshness and whitespace checks pass. The full learning objective remains incomplete; these results do not certify exhaustive reading coverage.

Linux compilation passed. All 3,315 bundled design files match the workspace byte-for-byte. The compiled engine hash matches current source: a15576d0e222169912ed2150d70b00cdfb1a0b37f01083bc82ed472afa5b8f39. The bundle contains only the four new opening cards and its vocabulary card in each opening section.
