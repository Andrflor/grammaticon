...
Tous les designs utilisent les widgets originaux de `lib/ui` et les scènes de `lib/game`, sans sélection d’une interface alternative. Les adaptateurs de présentation lisent les instantanés du moteur ; ils ne génèrent pas de questions. `theme.paintedPalette` associe chaque jeton de couleur du style original à sa valeur ARGB ; les polices viennent également du thème. Les coordonnées et largeurs des bâtiments, titres, sous-titres, bulles, portraits et sons proviennent des JSON. La palette comprend aussi les nuances des dégradés, bordures, ombres et voiles : changer seulement la couleur principale ne recolore pas ces jetons distincts. Le test `theme_test.dart` vérifie que chaque jeton utilisé est présent et que les boutons prennent les valeurs du JSON. Les couleurs intégrées aux images restent celles des assets référencés par le design.
